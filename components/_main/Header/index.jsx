...
function Header() {
  ...
  const getUserAvatarUrl = () => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(
      (user?.firstName || "") + " " + (user?.lastName || "")
    )}&size=128&background=random&color=fff&rounded=true&bold=true`;
  };
                ...
                <>
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <button
                      ...
                      className="flex items-center space-x-2 hover:opacity-80 transition-opacity max-w-[200px]"
                    >
                      <span className="font-semibold text-sm truncate">
                        {truncateText(user?.firstName, 12)}
                      </span>
                    ...
                    {showProfileDropdown && (
                      <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-100 py-2 z-50">
                        ...
                        <div className="py-1 max-h-60 overflow-y-auto">
                          {dropdownItems.map((item, index) => (
                            <Link
                              ...
                              href={item.href}
                              onClick={() => setShowProfileDropdown(false)}
                              className="flex items-center space-x-3 px-4 py-2 text-sm text-gray-700 hover:bg-[#11F2EB] hover:text-white transition-colors min-w-0"
                            >
                        ...
                        <div className="border-t border-gray-100 pt-1">
                          <button
                            onClick={handleLogout}
                            className="flex items-center space-x-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors min-w-0"
                          >
                    ...
                    <div className="pt-4 border-t border-gray-200">
                      ...
                      {dropdownItems.map((item, index) => (
                        <Link
                          ...
                          href={item.href}
                          onClick={() => setMobileMenuOpen(false)}
                          className="flex items-center space-x-3 px-4 py-3 text-sm text-gray-700 hover:bg-[#11F2EB] hover:text-white transition-colors min-w-0"
                        >
                          <item.icon className="h-4 w-4 text-gray-500 flex-shrink-0" />
                          <span className="truncate">{item.label}</span>
                        </Link>
                      ...
                      <button
                        onClick={handleLogout}
                        className="flex items-center space-x-3 w-full px-4 py-3 text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-colors mt-2 min-w-0"
                      >
                        <LogOut className="h-4 w-4 flex-shrink-0" />
                        <span className="truncate">Logout</span>
                      </button>

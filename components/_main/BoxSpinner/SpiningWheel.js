...
import { Button } from "@/components/Button";
import { useDispatch } from "react-redux";
import { Hexagon, Loader, RefreshCw, Zap, Square } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function SpinningWheel({
  ...
  const [currentSpeed, setCurrentSpeed] = useState(0);
  const [showConfetti, setShowConfetti] = useState(false);
  const [isShippingItem, setIsShippingItem] = useState(false); // NEW: Loading state for shipping
  const spinnerRef = useRef(null);
  const winnerIndexRef = useRef(0);
  const animationRef = useRef(null);
  const startTimeRef = useRef(0);
  ...
  const verifyWinnerPosition = (currentRotation, winnerIndex) => {
    ...
    return distanceFromWinningSpot < 1;
  };
  ...
  const resetSpinningWheel = () => {
    console.log("🔄 Resetting spinning wheel state...");
    ...
    console.log("✅ Spinning wheel reset complete");
  };
  ...
  // UPDATED: Use reset function instead of page reload
  const handleModalClose = () => {
    setShowConfetti(false);
    setShowModal(false);

    // Use setTimeout to ensure modal closing animation completes before reset
    setTimeout(() => {
      resetSpinningWheel();
    }, 300);
  };
  ...
  }, [isSpinning, winningItem, items, angleStep, isMounted]);
  ...
  // UPDATED: Added loading state to handleOrderWonItem
  const handleOrderWonItem = async (wonItem, spinData) => {
    if (wonItem && spinData) {
      setIsShippingItem(true); // Start loading
      try {
        dispatch(
          addWonItemToCart({
            item: wonItem,
        ...
        toastError("Something went wrong");
        setIsShippingItem(false); // Stop loading on error
      }
    } else {
      toastError("Something went wrong");
      setIsShippingItem(false); // Stop loading on error
              ...
              <div className="flex flex-col items-center pt-6 pb-4 px-6 relative z-10">
                ...
                {!isDemoSpin && (
                  <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
                    ...
                    {/* UPDATED: Added loading state to SHIP ITEM button */}
                    <Button
                      className="w-full h-11 bg-gradient-to-r from-[#11F2EB] to-cyan-600 
       hover:from-cyan-500 hover:to-[#11F2EB] text-slate-800 font-medium rounded-lg shadow-sm"
                      onClick={() =>

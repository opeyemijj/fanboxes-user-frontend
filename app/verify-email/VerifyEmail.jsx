...
export default function VerifyEmailContent() {
  ...
  const [urlCheckComplete, setUrlCheckComplete] = useState(false);
  const [hasInvalidUrlOtp, setHasInvalidUrlOtp] = useState(false);
    ...
    if (encodedOtp) {
      try {
        ...
        if (/^\d{6}$/.test(decodedOtp)) {
          ...
          toastError(
            "Invalid verification link. Please enter the code manually."
          );
          setHasInvalidUrlOtp(true);
      ...
      // No encoded OTP in URL - show manual entry immediately
      setIsCheckingUrl(false);
      setUrlCheckComplete(true);
    }
  }, [encodedOtp]);
  ...
  const handleAutoSubmit = async (otpValue) => {
    ...
    try {
      ...
      toastError("Verification failed. Please try entering the code manually.");
    } finally {
      setIsLoading(false);
      setIsCheckingUrl(false);
      setUrlCheckComplete(true);
  ...
  // Show loader while checking URL (only when there's actually an OTP parameter)
  if (isCheckingUrl && encodedOtp) {
    return (
  ...
  if (encodedOtp && verificationStatus && !hasInvalidUrlOtp) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col">
        <main className="flex-1 flex items-center justify-center p-4">
          <div className="w-full max-w-md">
  ...
  // Manual OTP verification flow - show in these cases:
  // 1. No OTP parameter in URL
  // 2. Invalid OTP parameter in URL
  // 3. Auto-verification failed and user clicked "Try manual entry"
  if (
    urlCheckComplete &&
    (!encodedOtp || hasInvalidUrlOtp || verificationStatus === "error")
  ) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex flex-col">
                ...
                {hasInvalidUrlOtp && (
                  <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-xl">
                    <p className="text-yellow-700 text-sm">
                      The verification link was invalid. Please enter the code
                      manually.
                    </p>
                  </div>
                )}

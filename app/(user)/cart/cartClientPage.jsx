...
import { ShoppingCart, Plus, Minus, Trash2, ArrowRight, Package, Loader } from "lucide-react"
import {
  selectCartTotal,
  selectShippingFee,
...
const CartClientPage = () => {
  ...
  const debouncedCartTotal = useDebounce(cartTotalValue, 500)
    ...
    try {
      ...
      if (res?.success && res?.data) {
        ...
        if (shippingRule.valueType === "percentage") {
          ...
          calculatedShippingFee = Number.parseInt(((itemsTotal * shippingRule.value) / 100).toFixed(0))
        } else {
          calculatedShippingFee = Number.parseInt(shippingRule.value)
        }
      }
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2">
              <div className="bg-white rounded-2xl shadow-sm p-6 animate-pulse">
                ...
                {[1, 2].map((item) => (
                  <div key={item} className="flex gap-4 mb-6 pb-6 border-b border-gray-100 last:border-b-0">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "WebPage",
            name: "Shopping Cart",
            description: "Review and manage your mystery box cart",
            url: `${process.env.NEXT_PUBLIC_SITE_URL || "https://fanboxes.com"}/cart`,
          }),
        }}
      />
      <div className="min-h-screen bg-gray-50 py-8 mt-20">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {uniqueItemCount} {uniqueItemCount === 1 ? "item" : "items"} in your cart
                </p>
              </div>
              <button
                      ...
                      <div className="flex gap-4">
                        <div className="flex-shrink-0 w-20 h-20 sm:w-24 sm:h-24 bg-gray-50 rounded-lg overflow-hidden">
                          <Image
                            ...
                            alt={`${item.name} - Mystery box product image`}
                            width={96}
                            height={96}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-4">
                            <div className="min-w-0 flex-1">
                              ...
                              {item.brand && <p className="text-gray-500 text-sm mt-1">{item.brand}</p>}
                              <div className="flex items-center gap-3 mt-2">
                                <span className="font-bold text-gray-900">{fCurrency(cCurrency(item?.value))}</span>
                                {item.weight && (
                          ...
                          <div className="flex items-center justify-between mt-4">
                            <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                              <button
                                onClick={() => handleQuantityUpdate(item.slug, item.quantity - 1)}
                            ...
                            <div className="text-right">
                              <p className="font-bold text-gray-900">
                                {fCurrency(cCurrency((item.value || 0) * item.quantity))}
                              </p>
                            </div>
                          </div>
                        </div>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  {discountApplied.type !== "none" && (
                    <div className="flex justify-between text-green-600">
                      <span>
                        Discount ({discountApplied.type === "percentage-off" ? `${discountApplied.amount}%` : "Fixed"})
                      </span>
                      <span>
                        -
                        {discountApplied.type === "percentage-off"
                          ? fCurrency(cCurrency(((subtotal + shippingFee) * discountApplied.amount) / 100))
                  ...
                  <div className="border-t border-gray-200 pt-3">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-lg font-bold text-gray-900">{fCurrency(cCurrency(totalAmount))}</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-8 bg-white rounded-2xl shadow-sm p-6">
            <div className="grid md:grid-cols-3 gap-6 text-center">
              <div className="flex flex-col items-center">
                ...
                <p className="text-sm text-gray-600">Quick & reliable delivery</p>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-12 h-12 bg-[#11F2EB]/10 rounded-full flex items-center justify-center mb-3">

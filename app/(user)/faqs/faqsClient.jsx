...
import { ChevronDown, ChevronUp, Loader, AlertCircle, Search } from "lucide-react"

// Helper component to safely render HTML content
const HtmlContent = ({ content, className = "" }) => {
  return <div className={`faq-answer-content ${className}`} dangerouslySetInnerHTML={{ __html: content }} />
}
...
// Helper function to parse HTML content into FAQs
    ...
    const addFAQ = (question, answer, index) => {
      if (question && answer.trim()) {
        ...
        faqs.push({
          ...
          answer: cleanAnswer,
          rawText: `${question.trim()} ${stripHtml(cleanAnswer)}`.toLowerCase(),
        })
      }
    }
    ...
    if (faqs.length === 0) {
      ...
      paragraphs.forEach((p, index) => {
        ...
        if (text && text.length > 50) {
          ...
          if (
            prevElement &&
            ["strong", "b", "h1", "h2", "h3", "h4", "h5", "h6"].includes(prevElement.tagName.toLowerCase())
          ) {
...
// Helper function to parse FAQs from any data structure
const parseFAQsFromData = (data) => {
  if (data?.faqs && Array.isArray(data.faqs) && data.faqs.length > 0) {
    return data.faqs.map((faq, index) => ({
      ...
      answer: faq.answer || "",
      rawText: `${faq.question || ""} ${stripHtml(faq.answer || "")}`.toLowerCase(),
    }))
  }
...
export default function FaqsClient() {
  ...
  const filterQuestions = () => {
    ...
    const filtered = allQuestions.filter((faq) => {
      const searchableText = faq.rawText || `${faq.question} ${stripHtml(faq.answer)}`.toLowerCase()
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Loader className="w-10 h-10 text-[#0DD4C7] animate-spin" />
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center p-4">
        <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2">Error Loading FAQs</h3>
          <p className="text-gray-600 mb-4">{error}</p>
          <button onClick={fetchFaqData} className="text-[#0DD4C7] hover:text-[#11F2EB] font-medium transition-colors">
            Retry
          </button>
        </div>
      </div>
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "@type": "FAQPage",
              mainEntity: displayFaqs.slice(0, 10).map((faq) => ({
                "@type": "Question",
                name: faq.question,
                acceptedAnswer: {
                  "@type": "Answer",
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
          ...
          <div className="space-y-4 mb-8">
            {hasQuestions ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100 text-center">
                ...
                <p className="text-gray-600 mb-4">We couldn't find any FAQs matching "{searchTerm}"</p>
                <button
                  onClick={() => setSearchTerm("")}
              ...
              <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-100 text-center">
                ...
                <p className="text-gray-600 mb-4">We're currently updating our frequently asked questions.</p>
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-8 mb-12 text-white text-center">
            ...
            <p className="text-slate-300 mb-6 max-w-2xl mx-auto">
              Can't find the answer you're looking for? Our support team is here to help you get the information you
              need.
            </p>

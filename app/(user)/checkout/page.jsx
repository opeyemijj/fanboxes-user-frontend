...
import { Package, Truck, CreditCard, Loader2, CheckCircle } from "lucide-react";
...
const CheckoutScreen = () => {
  ...
  // FIX: Provide proper default values for settings
  ...
  // FIX: State initialization with proper defaults
  ...
  // FIX: Check cart emptiness only after mount with proper fallback
  const isCartEmpty = mounted ? !cart.items || cart.items.length === 0 : true;

  // FIX: Conditional console.log to prevent hydration mismatch
  useEffect(() => {
    if (mounted) {
  ...
  }, [mounted, settings.currency, settings.rate]);
  ...
  const handleCompleteOrder = async () => {
    ...
    try {
      const orderPayload = {
        ...
        taxApplied: {
          percentage: isPostSpinOrder ? "0%" : "0%",
          amount: isPostSpinOrder ? 0 : 0,
        },
  ...
  // FIX: Show loading skeleton until mounted
  if (!mounted) {
    return (
